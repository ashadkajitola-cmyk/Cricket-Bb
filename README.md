...
<body class="bg-slate-100 font-sans text-gray-800">
    ...
    <div class="container mx-auto p-4 max-w-5xl">
        ...
        <div id="dashboard-section" class="hidden-section">
            ...
            <div class="flex flex-wrap gap-2 mb-6 border-b pb-2">
                ...
                <button onclick="switchTab('unlimited_shop')" class="px-3 py-2 bg-gray-200 text-gray-700 rounded font-medium text-sm tab-btn" id="btn-unlimited_shop">♾️ Unlimited Pts Vouchers</button>
            ...
            <div id="tab-shop" class="hidden-section bg-white p-6 rounded-lg shadow-md space-y-4">
                <h3 class="text-xl font-bold text-emerald-800">🛍️ Spend Diamonds / Balance for Vouchers & Subscriptions</h3>
                <p class="text-sm text-gray-600">Apne balance ya diamonds ka istemal karke naye voucher ya subscription code generate karein.</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="p-4 border rounded shadow-sm bg-gray-50">
                        <p class="font-bold text-lg text-emerald-800">Half Monthly Subscription</p>
                        <p class="text-sm text-gray-600 mb-3">Cost: ₹200 Balance</p>
                        <button onclick="buyVoucherOrSubFromBalance(200, 'sub_half_monthly', 0, 'Half Monthly Sub')" class="bg-blue-600 text-white px-4 py-2 rounded text-sm font-bold hover:bg-blue-700 w-full">Redeem for ₹200</button>
                    </div>
                </div>
            </div>

            <!-- TAB 5: UNLIMITED POINTS VOUCHERS (NEW SYSTEM) -->
            <div id="tab-unlimited_shop" class="hidden-section bg-white p-6 rounded-lg shadow-md max-w-xl mx-auto space-y-4 border-t-4 border-blue-600">
                <h3 class="text-xl font-bold text-blue-800">♾️ Unlimited Points Vouchers Generator</h3>
                <p class="text-sm text-gray-600 leading-relaxed bg-blue-50 p-3 rounded border border-blue-200">
                    <b>Niyam:</b> Aap jitna chaho utne points ka voucher **unlimited baar** kharid sakte hain. Badle mein aapke <b>₹ Balance (Rupees)</b> ka bhugtan hoga.
                </p>

                <div id="unlimited-voucher-box" style="display: none;" class="bg-amber-50 border border-amber-300 p-4 rounded mb-4">
                    <p class="text-xs text-gray-600 mb-1">Aapka Unlimited Generated Code:</p>
                    <div class="flex items-center justify-between bg-white p-2 border rounded">
                        <span id="unlimited-gen-code" class="font-bold text-blue-700 text-base select-all"></span>
                        <button onclick="copyTextById('unlimited-gen-code')" class="bg-blue-600 text-white text-xs px-3 py-1 rounded">Copy Code</button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="border p-4 rounded-lg bg-gray-50 flex flex-col justify-between">
                        <div>
                            <h4 class="font-bold text-blue-900">100 Pts Unlimited Voucher</h4>
                            <p class="text-xs text-gray-600 mb-2">Cost: ₹10 Balance</p>
                        </div>
                        <button onclick="buyUnlimitedPointsVoucher(10, 100, '100 Pts Unlimited Voucher')" class="bg-blue-600 text-white py-2 rounded text-sm font-bold hover:bg-blue-700 mt-2">Kharidein (₹10)</button>
                    </div>

                    <div class="border p-4 rounded-lg bg-gray-50 flex flex-col justify-between">
                        <div>
                            <h4 class="font-bold text-blue-900">500 Pts Unlimited Voucher</h4>
                            <p class="text-xs text-gray-600 mb-2">Cost: ₹45 Balance</p>
                        </div>
                        <button onclick="buyUnlimitedPointsVoucher(45, 500, '500 Pts Unlimited Voucher')" class="bg-blue-600 text-white py-2 rounded text-sm font-bold hover:bg-blue-700 mt-2">Kharidein (₹45)</button>
                    </div>

                    <div class="border p-4 rounded-lg bg-gray-50 flex flex-col justify-between">
                        <div>
                            <h4 class="font-bold text-blue-900">1000 Pts Unlimited Voucher</h4>
                            <p class="text-xs text-gray-600 mb-2">Cost: ₹85 Balance</p>
                        </div>
                        <button onclick="buyUnlimitedPointsVoucher(85, 1000, '1000 Pts Unlimited Voucher')" class="bg-blue-600 text-white py-2 rounded text-sm font-bold hover:bg-blue-700 mt-2">Kharidein (₹85)</button>
                    </div>

                    <div class="border p-4 rounded-lg bg-gray-50 flex flex-col justify-between">
                        <div>
                            <h4 class="font-bold text-blue-900">5000 Pts Mega Voucher</h4>
                            <p class="text-xs text-gray-600 mb-2">Cost: ₹400 Balance</p>
                        </div>
                        <button onclick="buyUnlimitedPointsVoucher(400, 5000, '5000 Pts Unlimited Voucher')" class="bg-blue-600 text-white py-2 rounded text-sm font-bold hover:bg-blue-700 mt-2">Kharidein (₹400)</button>
                    </div>
                </div>
            </div>
            ...
            <div id="tab-addcoins" class="hidden-section bg-white p-6 rounded-lg shadow-md max-w-lg mx-auto">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="user-voucher-code" placeholder="e.g. VCH-... or UNVCH-..." class="w-full p-2 border rounded focus:ring-2 focus:ring-emerald-500 uppercase">
                </div>
    ...
    <!-- JavaScript Logic -->
    <script>
        const DB_KEY = "cricket_portal_db_v17";
        ...
        function switchTab(tabName) {
            ...
            ['matches', 'points', 'diamonds', 'shop', 'unlimited_shop', 'subscription', 'addcoins', 'admin'].forEach(t => {
        ...
        // --- UNLIMITED POINTS VOUCHERS SYSTEM ---
        function buyUnlimitedPointsVoucher(costRupees, voucherPoints, itemName) {
            let db = getDB();
            let user = db.users[currentUserPhone];

            if (!user.rupees || user.rupees < costRupees) {
                alert(`Aapke paas paryapt ₹ Balance nahi hai! (Required: ₹${costRupees}, Available: ₹${user.rupees || 0}). Kripya "Buy Diamonds" tab se balance top-up karein.`);
                return;
            }

            user.rupees -= costRupees;
            if (db.userIDs[user.userid]) {
                db.userIDs[user.userid].rupees = user.rupees;
            }

            if (!db.customVouchers) db.customVouchers = {};
            let unlimitedCode = "UNVCH-" + user.userid.toUpperCase() + "-" + Math.floor(1000 + Math.random() * 9000);

            db.customVouchers[unlimitedCode] = {
                target: "any", 
            ...
            document.getElementById("unlimited-voucher-box").style.display = "block";
            document.getElementById("unlimited-gen-code").innerText = unlimitedCode;
            alert(`Badhai ho! "${itemName}" safaltapurvak kharid liya gaya hai. Aap jitni baar chahein ise repeat kar sakte hain.`);
        }
        ...
        function buyVoucherOrSubFromBalance(costInRupees, itemType, pointsValue, itemName) {
            ...
            let voucherCode = "";
            if (itemType === 'points') {
                voucherCode = "VCH-" + user.userid.toUpperCase() + "-" + Math.floor(1000 + Math.random() * 9000);
                if (!db.customVouchers) db.customVouchers = {};
                db.customVouchers[voucherCode] = { target: user.userid.toLowerCase(), points: pointsValue, used: false };
            } else if (itemType === 'sub_half_monthly') {
                let durationMs = 15 * 24 * 60 * 60 * 1000;
                let now = new Date().getTime();
                if (db.subscription.activeUntil > now) db.subscription.activeUntil += durationMs;
                else db.subscription.activeUntil = now + durationMs;
                voucherCode = "SUBHM-" + user.userid.toUpperCase() + "-" + Math.floor(1000 + Math.random() * 9000);
            }
        ...
        function userRedeemVoucher() {
            ...
            // Check if it's an unlimited code (target === "any") or specific user voucher
            if (vchData.target !== "any" && currentUser.phone.toLowerCase() !== vchData.target && currentUser.userid.toLowerCase() !== vchData.target) {
